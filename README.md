...
I just try to find an algorithm for WAL. And somehow my TLA+ model (independently) specifies roughly how InnoDB actually
does it, where:

- PageNum in my TLA+ Model => `hdr_no` in InnoDB log block header
- Generation in my TLA+ Model => `epoch_no` in InnoDB log block header
